...
const InvalidDatabaseParameterError = Error(
  'registerUser build(): Invalid database parameter'
);
const InvalidMesageStoreParameterError = Error(
  'registerUser build(): Invalid messageStore parameter'
);
...
function createActions({ messageStore, queries }) {
  ...
  function registerUser(traceId, attributes) {
    ...
    if (
      ...
      throw new TypeError(
        'registerUser() requires two arguments: a string and an object'
      );
    }
    ...
    return Bluebird.resolve(context)
      ...
      .catch((e) => {
        throw new Error('registerUser() error: ' + e?.message)
      });
  }
